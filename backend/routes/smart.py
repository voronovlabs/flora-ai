...
Flow:
  ...
  3. Strip hallucinated `filters.source` unless user explicitly mentioned
     a competitor (matches legacy behavior).
...
from backend.config import KNOWN_SOURCES, OPENAI_API_KEY, OPENAI_MODEL, SMART_MAX_ROWS
...
def smart(q: SmartQuery):
    ...
    if not OPENAI_API_KEY:
        ...
        res = ask(Question(question=question, preset=intent))
    ...
    try:
        ...
        has_known_source = any(k in q_low for k in KNOWN_SOURCES)
        ...
        sql_text = enforce_limit(sql_text, SMART_MAX_ROWS)
        data = run_sql_text(sql_text, params=params, limit=SMART_MAX_ROWS)
